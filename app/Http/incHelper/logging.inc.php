...
// ----------------------------------------------
			...
			$nrofrows1 = count($result1);
			//$nrofrows1 = mysql_affected_rows($mydb);

			$net2ftp_globals["log_access_id"] = $result1;
...
// -------------------------------------------------------------------------
    ...
    try{
	$result1   = DB::select("$sqlquery1") ;    }catch (Exception $xception){
        print_r($xception->getMessage());
    }
...
// -------------------------------------------------------------------------
	...
	if ($nrofrows1 == 1) { 
        ...
        $result2   =  DB::update("$sqlquery2");
        $nrofrows2 = count($result2);
	}
...
// Execute the step
		for ($i=1; $i<=sizeof($tables); $i++) {
			...
			$result_rename[$i] = DB::query("$sqlquery_rename");
