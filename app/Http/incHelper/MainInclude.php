...
public static function net2ftp($action)
	{
...
// -------------------------------------------------------------------------
		...
		$net2ftp_globals["application_includesdir"] = $net2ftp_globals["application_rootdir"] . "/../incHelper";
...
// Note: the error reporting level is already set to none in index.xml.php, to avoid XML parsing errors
			if (substr($_SERVER["PHP_SELF"], strlen($_SERVER["PHP_SELF"]) - 8, 8) == ".xml.php") {
				error_reporting(0);
			} elseif ($net2ftp_settings["error_reporting"] == "ALL") {
...
// 1. Libraries which are always needed
			require_once($net2ftp_globals["application_includesdir"] . "\\authorizationsInclude.php");
...
// ------------------------------------
		if ($net2ftp_globals["state"] == "admin" ||
			...
			if ($action == "sendHttpHeaders") {
				...
				if ( MainInclude::isStatusbarActive() == true) {
...
// **                                                                                  **
	...
	public static  function isStatusbarActive()
	{
